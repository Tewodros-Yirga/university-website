...
<head>
  ...
  if (isset($_POST['change'])) {
    ...
    $checkName = mysqli_query($connection, "select email from users where email = '$exname'");
    $res = mysqli_num_rows($checkName);
    if ($res > 0) {
      $chang = mysqli_query($connection, "update users set email = '$nwname' where email = '$exname'");
      if ($chang) {
...
<body>
  <div class="container">
    ...
    <form action="" method="post">
      <input type="email" placeholder="Enter existing user email" name="existingname" /><br />
      <input type="email" placeholder="Enter new user email" name="newname" /><br />
